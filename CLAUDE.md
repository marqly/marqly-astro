...
- 2026-08-05: apex `marqly.com` serves the site with **200 + byte-identical HTML** to `www` (no 301). Canonical tags point at www so Google consolidates, but it doubles crawl surface. `_redirects` is path-matched and CANNOT match hostname, and the worker can't intercept it (see the note above), so this is **zone config only**: Rules → URL Forwarding → Redirect Rules, `hostname eq "marqly.com"` → 301 `https://www.marqly.com${http.request.uri.path}`. The wrangler OAuth token only has `zone (read)`, so an agent cannot create this — it needs the dashboard.
