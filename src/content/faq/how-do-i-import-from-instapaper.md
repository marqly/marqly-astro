...
1. Export your articles from Instapaper — use the HTML export, since that's the standard bookmark format.
2. Sign in at [app.marqly.com](https://app.marqly.com); the free tier requires no credit card.
3. Import the HTML file into Marqly.

Marqly imports standard bookmark HTML — the same format browsers like Chrome, Firefox, Edge, and Safari export — so the Instapaper HTML export slots straight into the same flow. After the import, AI tagging runs automatically over every article. No folders to rebuild, no manual sorting session.

## What you gain over a classic read-it-later queue
...
Many Instapaper users also have a Pocket export from that service's 2025 shutdown, plus years of browser bookmarks. Marqly imports all three sources — see [how do I import from Pocket](/faq/how-do-i-import-from-pocket) and [how do I import Chrome bookmarks](/faq/how-do-i-import-chrome-bookmarks) — so everything ends up in one searchable place.
